            ...
            return (
              ...
              >
                ...
                item.name !== undefined ? (
                  ...
                  ) => React.ReactNode)(item.value as number | string, item.name, item, index, item.payload ?? {})
                ) : (
                  <>
                    {itemConfig?.icon ? (
                      <itemConfig.icon />
...
>(
  (
    ...
    return (
      ...
      >
        {payload.map((item) => {
          const key = `${nameKey ?? (item as PayloadItem).dataKey ?? "value"}`;
          const itemConfig = getPayloadConfigFromPayload(config, item, key);

          return (
            <div
              key={String((item as PayloadItem).value)}
              className={cn(
            ...
            >
              {itemConfig?.icon && !hideIcon ? (
                ...
                <div
                  ...
                  style={{
                    backgroundColor: (item as PayloadItem).color,
                  }}
                />
              )}
              {itemConfig?.label}
